...
#include <atlbase.h>
//You may derive a class from CComModule and use if you want to override something,
...
//initialized at WM_SIZE
    ...
    std::string szProjectFile; //path and filename of project file
    std::string CurTitle;  //hols the name without path of file in editor window
    std::string szExecutableFile;

    std::string szCompilerPath;// = TEXT("c:\\Borland\\bcc55\\bin\\bcc32.exe"); //Path to compilerstatic
    std::string szResourcePath;// = TEXT("c:\\Borland\\bcc55\\bin\\brcc32.exe");// Path to resource compiler
    std::string szLinkerPath;// = TEXT("c:\\Borland\\bcc55\\bin\\ilink32.exe"); // path to linker
    std::string psdkPath;//     = TEXT("c:\\Borland\\bcc55\\lib\\psdk");        //lib path
    std::string IncludePath;//  = TEXT("c:\\Borland\\bcc55\\include");           //include path
    std::string LibPath;//      = TEXT("c:\\Borland\\bcc55\\lib");
    std::string CompilerArgs; //these are the std::strings passed to the compiler, linker and resource compiler
    std::string ResourceArgs;
    std::string LinkerArgs;
    std::string g_CurrentDir; // for holding the current directory or project file directory
    std::string addlibraries; //place to add aditional libraries to the linker
    std::string szProjectName; //name of project without path
    ...
    BOOL bModify;
    BOOL g_bCompilerStatus; //compiler std::strings have been or not initialized
    BOOL bColorsModify;
    int iErrors;
    int iWarnings;
    BOOL brunning;
    ...
    HWND hwndSelected;
    std::vector <std::string> VectorSource; //holds source files names(*.cpp or *.c) //holds source files names(*.cpp or *.c)
    std::vector <std::string> VectorHeader; // holds headers files (*.h) //
    std::vector <std::string> VectorOther;  // holds others files actually only text files (*.txt)
    std::vector <std::string> VectorResource;
    std::map<std::string, HWND > Mem_map;
    std::map <std::string, char> file_map;
